...
export class PIXIMovieClip extends KFActor implements PIXIObject
{
    ...
    ///KFD(P=1,NAME=eventDown,CNAME=点击事件,TYPE=bool,DEFAULT=false)
    public eventDown:boolean;

    ///KFD(P=2,NAME=eventMove,CNAME=移动事件,TYPE=bool,DEFAULT=false)
    public eventMove:boolean;

    ///KFD(P=3,NAME=eventTick,CNAME=TICK事件,TYPE=int32,DEFAULT=-1)
    public eventTick:number;

    ///KFD(P=4,NAME=autoStateID,CNAME=进入状态,TYPE=int32,DEFAULT=-1)
    public autoStateID:number;

    ///KFD(P=5,NAME=bGraphic,CNAME=图形模式,TYPE=bool,DEFAULT=false)
    ...
    public ActivateBLK(KFBlockTargetData: any): void
    {
        ...
        if(this.eventDown) {
            this._mEmit = new PIXIMouseEventEmit(this._container,this.etable,this.eventMove);
        }
